...
list = open('specflatg4s1.list')
bigflat = numpy.zeros((nframes,ysize,xsize),float)
BIASframe = fits.open('specBIAS.fits')
BIAS = numpy.array(BIASframe[0].data)
for i in range(0,nframes):
   print 'Image number:', i
   rawflat = fits.open(rstrip(list.readline()))
   print 'Info on file:'
   print rawflat.info()
...
fits.writeto('specflatg4s1.fits',medianflat,hdr,overwrite=True)
